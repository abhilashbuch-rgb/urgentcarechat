...
comment on table staff.eod_billing_reports is
  'One row per billing specialist per day. Who may write is app/api/staff/billing-report/route.ts''s concern, not RLS''s.';
...
-- ========== staff-billing-specialist-addon.sql ==========
...
-- billing_specialist seat add-on above started before it needed real
-- billing. Everything below stays inert, including the nav link, until
-- the flag is on.
--
