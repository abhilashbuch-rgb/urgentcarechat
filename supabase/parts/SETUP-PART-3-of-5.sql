...
-- finished the task. The key is stable across occurrences, so the token
-- names that instead and the feed always resolves to whichever row is
...
-- working indefinitely until an administrator revokes it.
-- ============================================================
...
-- SECURITY DEFINER, same reasoning as staff.redeem_surveyor_token(). A
-- calendar app has no session and therefore no org context; the org and
-- key are the ANSWER to this function, not an input to it.
